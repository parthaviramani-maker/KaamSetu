...
import { MdClose, MdLock, MdQrCode, MdCheckCircle, MdWarning, MdVpnKey } from 'react-icons/md';
  ...
  const handleSetPassword = async (e) => {
    ...
    try {
      ...
      if (errCode === 'ALREADY_HAS_PASSWORD') {
        // User already has a password — just send them back to the form to use it
        toast.info('You already have a password. Enter it below to add money.');
        setStep('form');
      } else {
        ...
        >
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', color: '#fff' }}>
              <MdQrCode size={22} />
              <div>
                <div style={{ fontWeight: 800, fontSize: '1rem' }}>Add Money to Wallet</div>
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  <div style={{ textAlign: 'center', marginBottom: '1.2rem' }}>
                    ...
                    <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', marginTop: '0.4rem' }}>
                      Your account doesn't have a password yet. Set one to verify wallet top-ups.
                    </p>
                  </div>
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '1.2rem' }}>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textAlign: 'center' }}>
                      ...
                      <strong style={{ color: 'var(--color-accent)' }}>kaamsetu@upi</strong>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', marginBottom: '1.1rem' }}>
                    ...
                    <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>or enter amount manually</span>
                    <div style={{ flex: 1, height: 1, background: 'var(--border-color)' }} />
                  </div>
                    ...
                    {PRESETS.map(p => (
                      <button
                        ...
                        style={{
                          ...
                          color: String(amount) === String(p) ? '#27ae60' : 'var(--text-secondary)',
                          fontSize: '0.78rem',
                          fontWeight: 600,
                          cursor: 'pointer',
                  ...
                  <form onSubmit={handleSubmit} noValidate>
                    ...
                    <div className="form-group" style={{ marginBottom: '1.2rem', position: 'relative' }}>
                      ...
                      <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: '0.3rem' }}>
                        Security check — your password is never stored in this flow.
                      </p>
                    </div>
                    ...
                    {Number(amount) > 0 && (
                      <div style={{
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <span style={{ color: 'var(--text-secondary)' }}>Adding:</span>
                          <strong style={{ color: '#27ae60' }}>+₹{Number(amount).toLocaleString('en-IN')}</strong>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.3rem' }}>
                          <span style={{ color: 'var(--text-secondary)' }}>New Balance:</span>
                          <strong>₹{(currentBalance + Number(amount)).toLocaleString('en-IN')}</strong>
                        </div>
                      </div>
                    )}
                    ...
                    <button
                      ...
                      disabled={isLoading}
                    >
                      {isLoading ? 'Verifying…' : `💳 Add ₹${Number(amount) > 0 ? Number(amount).toLocaleString('en-IN') : '—'} to Wallet`}
                    </button>
                  </form>
                </motion.div>
              )}
              ...
              {step === 'success' && (
                ...
                >
                  ...
                  <h2 style={{ color: '#27ae60', marginBottom: '0.4rem' }}>
                    ₹{addedAmount.toLocaleString('en-IN')} Added!
                  </h2>
                  ...
                  <p style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--text-primary)' }}>
                    ₹{(currentBalance + addedAmount).toLocaleString('en-IN')}
                  </p>
          ...
          {step === 'form' && (
            <div style={{
              ...
              Demo project — QR is for display only. Password verified securely with bcrypt.
            </div>
          )}
